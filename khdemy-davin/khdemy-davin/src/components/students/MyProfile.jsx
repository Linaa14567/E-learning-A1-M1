...
import { useUpdateMeMutation } from '../../features/api/userApi'
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-black text-slate-900 leading-tight" style={{ fontFamily: "'Outfit',sans-serif" }}>
              {form.name || "—"}
            </h2>
            <p className="text-sm font-semibold text-indigo-500 mb-2">{ROLE}</p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input value={form.dob} readOnly
              placeholder="DD / MM / YYYY" className={inputCls + " bg-slate-50 cursor-not-allowed text-slate-400"} />
          </div>
          <div>
            <label className={labelCls}>Email</label>
            <input value={form.email} readOnly
              placeholder="your@email.com" className={inputCls + " bg-slate-50 cursor-not-allowed text-slate-400"} />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.phone} readOnly
              placeholder="+855 xx xxx xxx" className={inputCls + " bg-slate-50 cursor-not-allowed text-slate-400"} />
          </div>
          <div className="col-span-1 sm:col-span-2">
            <label className={labelCls}>Bio</label>
            <textarea value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))}
              rows={3} placeholder="Tell us about yourself…" className={inputCls + " resize-none"} />
          </div>
        </div>

        <div className="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-slate-100">
          {saved && (
            <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-xl">✓ Saved!</span>
          )}
          {errMsg && (
            <span className="text-xs font-bold text-rose-600 bg-rose-50 px-3 py-1.5 rounded-xl">{errMsg}</span>
          )}
