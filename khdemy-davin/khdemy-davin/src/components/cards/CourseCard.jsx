  ...
  return (
    ...
    >
      <div className="relative w-full h-[160px] sm:h-[190px] overflow-hidden flex-shrink-0 bg-gray-200">
        <img src={c.img} alt={c.title} className="w-full h-full object-cover block" onError={e => { e.target.style.display = "none" }} />
        <span className="absolute top-3 left-3 bg-[#2F327D] text-white text-xs font-bold rounded-full px-3 py-1 tracking-wide">{c.cat}</span>
        {type === "completed" && (
          <span className="absolute top-3 right-3 bg-emerald-500 text-white text-xs font-bold rounded-full px-3 py-1">Grade {c.grade}</span>
        )}
        {onToggleSave && (
      ...
      <div className="p-3 sm:p-[18px] flex flex-col flex-1">
        <h3 className="font-bold text-sm sm:text-base text-center mb-1.5 sm:mb-2 leading-snug text-gray-900 line-clamp-2">{c.title}</h3>
        <p className="text-base sm:text-[17px] text-center leading-relaxed mb-3 sm:mb-4 flex-1 line-clamp-2 text-gray-400">{c.description || c.desc || c.instructor}</p>
        ...
        {type === "completed" && (
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <StarIcon /><span className="font-bold text-xs sm:text-[13px] text-gray-400">5.0</span>
              {onToggleSave && (
                <button onClick={e => { e.stopPropagation(); onToggleSave(c.id) }} className="bg-transparent border-none cursor-pointer p-0 leading-none ml-1">
                  <svg width="15" height="19" viewBox="0 0 16 20"
        ...
        {type === "saved" && (
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <StarIcon /><span className="font-bold text-xs sm:text-[13px] text-gray-400">{c.rating || "4.8"}</span>
              {onToggleSave && (
                <button onClick={e => { e.stopPropagation(); onToggleSave(c.id) }} className="bg-transparent border-none cursor-pointer p-0 leading-none ml-1">
                  <svg width="15" height="19" viewBox="0 0 16 20"
                    className={"block transition-all duration-200 " + (isSaved ? "fill-[#2F327D] stroke-[#2F327D]" : "fill-none stroke-gray-400")}
